...
/*
	$('[data-scroll-to]').on('click', function (e) {
		const target = jQuery(this).data('scroll-to')
		console.log(e);
		e.stopPropagation();
		// console.log(tokyo_tm_page_scroll_to);
		console.log('@1', target);
		tokyo_tm_page_scroll_to(target, 'slow', 0);
		return false;
	})
*/
...
function tokyo_tm_page_scroll_to(target, duration, customOffset){
console.log('@tokyo_tm_page_scroll_to')
	"use strict";

	if (typeof target === 'number') {
		// nothing
	} else if (typeof target === 'string' && jQuery(target).length) {
		target = jQuery(target).eq(0).offset().top
	} else if (typeof target === 'object' && jQuery(target).length) {
		target = jQuery(target).eq(0).offset().top
	} else if (target instanceof jQuery && target.length) {
		target = target.eq(0).offset().top
	} else {
		console.log('@target',typeof target, jQuery(target).length);
		return false
	}
...
function tokyo_tm_contact_form(){
	...
	"use strict";
	console.log('tokyo_tm_contact_form')
	
	jQuery("#BasicContactForm-6").on('submit', function(e){
		e.stopPropagation();
		var $name 		= jQuery(".contact_form #basicContactForm-6-name").val();
		var $email 		= jQuery(".contact_form #basicContactForm-6-email").val();
		var $message 	= jQuery(".contact_form #basicContactForm-6-message").val();
		var $success     = jQuery(".contact_form .returnmessage").data('success');
		var $form = jQuery(".contact_form form#BasicContactForm-6");
		var $container = $form
		var waitingForResponse = false;

		var action = jQuery(".contact_form form").attr('action');
		...
		if($name===''||$email===''||$message===''){
			...
			jQuery.ajax({
				...
				success: (response) => {
					var $html = $('#BasicContactForm-6', response)
			...
			/*jQuery.post(action,{ ajax_name: name, ajax_email: email, ajax_message:message, ajax_subject: subject},
				function(data) {
